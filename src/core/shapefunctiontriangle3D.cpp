...
void ShapeFunctionTriangleSigned::calc()
{
    ...
    VectorX<double> f(dim), p1(dim), p2(dim), p3(dim);
    ...
    VectorX<double> f1(dim), f2(dim), f3(dim); // calculate vectors from point f to vertices p1, p2 and p3:
    f1.subs( p1, f);
    f2.subs( p2, f);
    f3.subs( p3, f);
    VectorX<double> va(dim), va1(dim), va2(dim), va3(dim);
    double a, a1, a2, a3;
    va.multElements(p1-p2, p1-p3);
    va1.multElements(f2, f3);
    va2.multElements(f3, f1);
    va3.multElements(f1, f2);
    VectorX<double> vaa1(dim), vaa2(dim), vaa3(dim);
    a = va.norm2();
