...
//  cout << "S_max = " << s_max << endl;
    ...
    if (dim == 1) {
        ...
        w.resize(4, nn);
        this->phi.resize(4, nn);
    }
    else { std::cout << "WARNING: Space dimension is over 3, and not implemented." << std::endl; }
    if (mm == 1) {
        m = dim + 1;
    }
...
{
//  double radius;
    double rr2, xn, yn = 0.0, zn(0), xp, yp = 0.0, zp(0), qq;
    //For case 1:
    double s_i = 0.0;
    double ss_i;
...
//                      + 4 * qq * qq * std::pow( yp - yn, 2 ) * std::exp( -qq * rr2);
    ...
    case 1:
        ...
        for (int j = 0; j < nn; ++j) {
            xn = gp->supportNodes[j]->getX();
            if (dim >= 2) yn = gp->supportNodes[j]->getY();
            if (dim == 3) zn = gp->supportNodes[j]->getZ();
            if (dim == 1) s_i = std::sqrt(std::pow(xn - xp, 2));
            if (dim == 2) {
            ...
            if (ss_i < .5) {
                ...
                if (dim >= 2) w(2, j) = (8. - 12. * ss_i) * (yn - yp) / (s_max * s_max);
                ...
                if (dim >= 2) w(2, j) = (4. / ss_i - 8. + 4. * ss_i) * (yn - yp) / (s_max * s_max);
...
//  }
    ...
    if (mm == 1) {
        for (int i = 0; i < nn; ++i) {
            ...
            if (dim >= 2) P.writeElement(gp->supportNodes[i]->getY(), i, 2);
            if (dim == 3) {
...
//  lmx::Vector<double> alpha_2(m); /**< auxiliary variables, y derivative */
    ...
    // PHI:
    {
        ...
        if (dim >= 2) {
            rhs(2) = gp->Y;
        } // base is shifted to GP, so rest of elements are zero.
        if (dim == 3)
            rhs(3) = gp->Z;
...
// 		     + (alpha[1](0) + alpha[1](1)*P(j,1) + alpha[1](2)*P(j,2))*w(0,j);
                        += (alpha[0](0) + alpha[0](1) * P(j, 1)) * w(1, j)
                           + (alpha[1](0) + alpha[1](1) * P(j, 1)) * w(0, j);
                if (dim >= 2) {
                    this->phi(1, j)
...
//        cout << "phi(1,"<<j<<") = " << phi(1,j) << endl;
        ...
        if (dim >= 2) {
            ...
            lsys.solveYourself();
            // phi,y
// 	    cout << endl << "System " << A[0] << alpha[2] << rhs << endl;

            for (int j = 0; j < nn; ++j) {
                this->phi(2, j)
//                 += (alpha[0](0) + alpha[0](1)*P(j,1) + alpha[0](2)*P(j,2))*w(2,j)
//                    + (alpha[2](0) + alpha[2](1)*P(j,1) + alpha[2](2)*P(j,2))*w(0,j);
                        += (alpha[0](0) + alpha[0](1) * P(j, 1)) * w(2, j)
                           + (alpha[2](0) + alpha[2](1) * P(j, 1)) * w(0, j);
                if (dim >= 2) {
                    this->phi(2, j)
        ...
        if (dim == 3) {
            ...
            lsys.solveYourself();
// 	    cout << endl << "System " << A[0] << alpha[3] << rhs << endl;
            // phi,z
            for (int j = 0; j < nn; ++j) {
                this->phi(3, j)
                ...
                this->phi(3, j)
                        += (alpha[0](3) * P(j, 3)) * w(3, j)
                           + (alpha[3](3) * P(j, 3)) * w(0, j);
            }
        }
    }
