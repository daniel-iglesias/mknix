...
//                                                  - gp->supportNodes[j]->getZ() , 2)
    ...
    case 10:
        ...
        for (int i=0; i<nn; ++i) {
            for (int j=0; j<nn; ++j) {
                radius = std::pow
                         ( std::pow( gp->supportNodes[i]->getX() - gp->supportNodes[j]->getX()
                                     , 2)
                           +std::pow( gp->supportNodes[i]->getY() - gp->supportNodes[j]->getY()
                                      , 2)
...
void ShapeFunctionRBF::computePhi(double xp, double yp, double zp)
{
    ...
    if(dim==2)
        ...
        rk.resize(4, nn); /**< Radial basis and derivatives. */
