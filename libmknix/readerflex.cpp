...
void mknix::ReaderFlex::readFlexBodies(System * system_in)
{
    ...
    while(*input >> bodyType) {
        if(bodyType=="ENDFLEXBODIES") break;
...
// 	      cout << "keyword: " << keyword << endl;
		  ...
		  double nGPs, alpha;
                    ...
                    *output << "BOUNDARYGROUP: " << keyword << endl;
                    while(*input >> keyword) {
		      if( keyword=="ENDBOUNDARYGROUP" ) break;
		      else if(keyword=="METHOD"){
			...
			int nodeNumber;
			  ...
			  *output << "\tNode\tx\ty\tz \n";
			  for (int i=0; i<totalNodes; ++i) {
			      ...
			      system_in->flexBodies[flexTitle]->addNodeToBoundaryGroup(nodeNumberInFile-1, boundaryName);
			      *output
	    ...
	    //           do {input->get(a);} while(a!='\n');
			  ...
			  *output << "\tPoint\tx\ty\tz \n";
			  for (int i=0; i<totalPoints; ++i) {
			      ...
			      theSimulation->outputPoints[old_size + i] =
				  new Point(Simulation::getDim(), old_size + i, x , y, z, alpha, dc); // dc is set to 1.03 for the moment
			      system_in->flexBodies[flexTitle]
