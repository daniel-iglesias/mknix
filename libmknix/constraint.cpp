...
{
  std::vector< lmx::Vector<data_type> >::iterator itForces;
  int i, vectorSize(internalForces.size());
  
  if( internalForcesOutput.size() > 0 ) {

    *outFile << "FORCES " << title << " " << vectorSize << endl;
    for( itForces = internalForcesOutput.begin();
	 itForces!= internalForcesOutput.end();
	 ++itForces
	)
    {
	for( i=0; i<vectorSize; ++i) {
	    *outFile << itForces->readElement(i) << " ";
	}
	*outFile << endl;
    }
  }
