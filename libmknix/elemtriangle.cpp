...
//   cout << "CellTriang points " << this->points << endl;
  ...
  this->createGaussPoints_MC();
    ...
    {
        for( i=0; i<3; ++i )
            (*it)->addSupportNode( dynamic_cast<Node*>(this->bodyPoints[i]) );
    }
...
void ElemTriangle::createGaussPoints_MC()
{
  ...
  else if(nGPoints == 3) nGPoints_MC = 7; // Why not 6?
  else if(nGPoints == 4) nGPoints_MC = 7;
  else if(nGPoints == 6) nGPoints_MC = 12;
  else if(nGPoints == 7) nGPoints_MC = 12;
