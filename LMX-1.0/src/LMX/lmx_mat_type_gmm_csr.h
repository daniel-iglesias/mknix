 ...
 ***************************************************************************/
#ifndef LMXDATA_TYPE_GMM_CSR_H
#define LMXDATA_TYPE_GMM_CSR_H
...
namespace lmx {
    ...
    */
template <typename T> class Type_gmm_csr : public Data_mat<T>{

private:
  /** Matrix data contents */
    gmm::csr_matrix<T> gmm_csr_matrix;
//
public:
  ...
  Type_gmm_csr(size_type, size_type);
  ...
  { static T reference = 0;
    return reference = gmm_csr_matrix(row, column);
  }
  ...
  {
    gmm_csr_matrix(row, column) = value;
  }
   ...
   */
  size_type getRows()const { return gmm_csr_matrix.nrows(); }

  /** Method for knowing the number of data columns.
   * \returns Number of columns.
   */
  size_type getCols() const { return gmm_csr_matrix.ncols();}
...
{ gmm_csr_matrix = static_cast<const Type_gmm_csr*>(matrix_in)->gmm_csr_matrix; }
  ...
  /** Multiply method.
   ...
   { gmm::mult(static_cast<const Type_gmm_csr*>(matrix_in_1)->gmm_csr_matrix,
               static_cast<const  Type_gmm_csr*>(matrix_in_2)->gmm_csr_matrix,
               gmm_csr_matrix);
   }
  ...
  {
    gmm::scale(gmm_csr_matrix, scalar);
  }
  ...
  void multiplyElements(const Data<T>* matrix_in)
   {
     for (size_type i=0; i<gmm_csr_matrix.nrows(); ++i){
       for (size_type j=0; j<gmm_csr_matrix.ncols(); ++j)
         gmm_csr_matrix(i,j) *= matrix_in->readElement(i,j);
     }
   }
   ...
   /** Read data in Matrix Market format method.
     ...
     { gmm::col_matrix< gmm::wsvector<double> > matrix_loaded;
       ...
       gmm::resize(gmm_csr_matrix, gmm::mat_nrows(matrix_loaded), gmm::mat_ncols(matrix_loaded));
       gmm::copy(matrix_loaded, gmm_csr_matrix);
     }
     ...
     {
       gmm::csr_matrix<double> matrix_loaded;
       gmm::Harwell_Boeing_load(input_file, matrix_loaded);
       gmm::resize(this->gmm_csr_matrix, gmm::mat_nrows(matrix_loaded), gmm::mat_ncols(matrix_loaded));
       gmm::copy(matrix_loaded, gmm_csr_matrix);
     }
    ...
    */
     ...
     {
       gmm::csr_matrix<double> csrmat( gmm::mat_nrows(this->gmm_csr_matrix), gmm::mat_ncols(this->gmm_csr_matrix) );
       gmm::copy(this->gmm_csr_matrix, csrmat);
       gmm::Harwell_Boeing_save(input_file, csrmat);
     }
     ...
     void cast_csr_matrix(gmm::csr_matrix<T> &matrix_to_cast)
     {
      ...
      void trn()
      {
        gmm::dense_matrix<T> temp( gmm::mat_ncols(gmm_csr_matrix), gmm::mat_nrows(gmm_csr_matrix) );

        copy( transposed(gmm_csr_matrix), temp );
        gmm::resize(gmm_csr_matrix, gmm::mat_nrows(temp), gmm::mat_ncols(temp));
        copy(temp, gmm_csr_matrix);
      }
  ...
  {
    gmm::clean(gmm_csr_matrix, factor);
  }
  ...
  /** Data pointer method
    ...
    template <typename T>
    Type_gmm_csr<T>::Type_gmm_csr(size_type rows, size_type columns) :
     Data_mat<T>()
      {
        gmm::resize(gmm_csr_matrix, rows, columns);
      }
    ...
    template <typename T>
    void Type_gmm_csr<T>:: resize(size_type mrows, size_type ncolumns)
      { gmm::resize(gmm_csr_matrix, mrows, ncolumns); }



};// namespace lmx

#endif//LMXDATA_TYPE_GMM_CSR_H
