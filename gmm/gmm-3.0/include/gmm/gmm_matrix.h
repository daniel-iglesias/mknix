...
template<typename M> inline bool is_identity(const M&)
{
    return false;
}
...
(std::ostream &o, const row_matrix<V>& m)
{
    gmm::write(o,m);
    return o;
}
...
template<typename V> std::ostream &operator <<
(std::ostream &o, const col_matrix<V>& m)
{
    gmm::write(o,m);
    return o;
}
...
(std::ostream &o, const dense_matrix<T>& m)
{
    gmm::write(o,m);
    return o;
}
...
{
    GMM_ASSERT1(false, "Sorry unsupported type");
    return MPI_FLOAT;
}
...
template <typename MAT> struct mpi_distributed_matrix
{
    ...
    const MAT &local_matrix(void) const
    {
        return M;
    }
    MAT &local_matrix(void)
    {
        return M;
    }
};

template <typename MAT> inline MAT &eff_matrix(MAT &m)
{
    return m;
}
template <typename MAT> inline
const MAT &eff_matrix(const MAT &m)
{
    return m;
}
template <typename MAT> inline
MAT &eff_matrix(mpi_distributed_matrix<MAT> &m)
{
    return m.M;
}
template <typename MAT> inline
const MAT &eff_matrix(const mpi_distributed_matrix<MAT> &m)
{
    return m.M;
}
