                              ...
                              const char* Rhstype, int shift)
{
    ...
    if ( Type[0] != 'P' )            /* Skip if pattern only  */
    {
        if ( Valfmt == NULL ) Valfmt = (char *)"(4E21.13)";
        ParseRfmt(Valfmt, &Valperline, &Valwidth, &Valprec, &Valflag);
                        ...
                        const csc_matrix_ref<T*, INDI*, INDJ*, shift>& A)
{
    ...
    if (is_complex_double__(T()))
        if (mat_nrows(A) == mat_ncols(A)) t = "CUA";
        else t = "CRA";
    else if (mat_nrows(A) == mat_ncols(A)) t = "RUA";
    else t = "RRA";
...
{
    HarwellBoeing_IO h(filename.c_str());
    h.read(A);
}
...
public:
    MatrixMarket_IO() : f(0) {}
    MatrixMarket_IO(const char *filename) : f(0)
    {
        open(filename);
    }
    ~MatrixMarket_IO()
